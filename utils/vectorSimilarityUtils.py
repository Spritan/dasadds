...
def compute_similarity_vector(prev_pose, current_pose, onlyHands=True):
    ...
    try:
        ...
        prev_pose_v_left_hips_Knee          = calculate_vector(prev_pose_left_hips, prev_pose_left_Knee)
        prev_pose_v_left_Knee_ankle         = calculate_vector(prev_pose_left_Knee, prev_pose_left_ankle)
        ...
        prev_pose_v_right_hips_Knee         = calculate_vector(prev_pose_right_hips, prev_pose_right_Knee)
        prev_pose_v_right_Knee_ankle        = calculate_vector(prev_pose_right_Knee, prev_pose_right_ankle)
        ...
        current_pose_v_left_hips_Knee       = calculate_vector(current_pose_left_hips, current_pose_left_Knee)
        current_pose_v_left_Knee_ankle      = calculate_vector(current_pose_left_Knee, current_pose_left_ankle)
        ...
        current_pose_v_right_hips_Knee      = calculate_vector(current_pose_right_hips, current_pose_right_Knee)
        current_pose_v_right_Knee_ankle     = calculate_vector(current_pose_right_Knee, current_pose_right_ankle)
        ...
        similarity_left_hips_Knee           = cosine_similarity(prev_pose_v_left_hips_Knee , current_pose_v_left_hips_Knee)
        similarity_left_Knee_ankle          = cosine_similarity(prev_pose_v_left_Knee_ankle  , current_pose_v_left_Knee_ankle )
        ...
        similarity_right_hips_Knee          = cosine_similarity(prev_pose_v_right_hips_Knee , current_pose_v_right_hips_Knee)
        similarity_right_Knee_ankle         = cosine_similarity(prev_pose_v_right_Knee_ankle , current_pose_v_right_Knee_ankle)
        ...
        # Average the similarities for a final similarity score
            ...
            similarity = (similarity_left_shoulder_elbow  + similarity_left_elbow_wrist  +
                          similarity_right_shoulder_elbow + similarity_right_elbow_wrist +
                          similarity_left_hips_Knee       + similarity_left_Knee_ankle   +
                          similarity_right_hips_Knee      + similarity_right_Knee_ankle  ) / 8
...
def find_most_similar_keypoints_vectors(primary_frames, student_keypoints, onlyHands):
    ...
    for id, primary_frame in enumerate(primary_frames):
        ...
        for idx in range(last_selected_index + 1, len(student_keypoints)):
            ...
            similarity = compute_similarity_vector(primary_frame, student_frame, onlyHands)
